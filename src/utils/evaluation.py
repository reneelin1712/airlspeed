...
def evaluate_model(target_od, target_traj, target_weather, model, env, n_link=424):
    ...
    model.to(device)  # Move the model to the CPU device
    # env.policy_mask = env.policy_mask.to(device)  # Move policy_mask to the CPU device
    # env.state_action = env.state_action.to(device)  # Move state_action to the CPU device
    ...
    action_prob = model.get_action_prob(state_ts, des_ts, weather_ts).detach().numpy()  # 714, 8
    state_action = env.state_action[:-1]  # Convert state_action to NumPy array
