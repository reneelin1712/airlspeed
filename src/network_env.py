...
Step = namedtuple('Step', ['cur_state', 'action', 'next_state', 'reward', 'done', 'speed'])
...
class RoadWorld(object):
    ...
    def reset(self, st=None, des=None, weather=None):
        if st is not None and des is not None and weather is not None:
            self.cur_state, self.cur_des, self.cur_weather = st, des, weather
        else:
            od_idx = np.random.choice(self.od_list, p=self.od_dist)
            self.cur_state, self.cur_des, self.cur_weather = map(int, od_idx.split('_'))
    ...
    def import_demonstrations(self, demopath, od=None, n_rows=None):
        ...
        for demo_str, demo_des, demo_weather in zip(demo['path'].tolist(), demo['des'].tolist(), demo['speed'].tolist()):
    ...
    def import_demonstrations_step(self, demopath, n_rows=None):
        ...
        for demo_str, demo_des, demo_weather in zip(demo['path'].tolist(), demo['des'].tolist(), demo['speed'].tolist()):
            ...
            for i0 in range(1, len_demo):
                ...
                episode.append(Step(cur_state=cur_state, action=action, next_state=next_state, reward=reward, done=is_done, speed=demo_weather))
